...
export function getDocByType(id: string) {
  return allDocs.filter((doc: any) => doc.slug.startsWith(`/docs/${id}`))
}
...
export function getGroupedComponents() {
  return getDocByType('components').reduce((acc: any, doc: any) => {
...
const getUsageDoc = (id: string) => {
  return allDocs.find((_doc: any) => _doc.id === id && _doc.scope === 'usage')
}

const getThemingDoc = (id: string) => {
  return allDocs.find((_doc: any) => _doc.id === id && _doc.scope === 'theming')
}

const getPropsDoc = (id: string) => {
  return allDocs.find((_doc: any) => _doc.id === id && _doc.scope === 'props')
}
...
export const getDocDoc = (slug: MixedArray): Doc | undefined => {
  ...
  const doc = allDocs.find(
    (doc: any) => doc.slug.endsWith(_slug) || doc.slug.endsWith(`${_slug}/usage`),
  ) as Doc | undefined
