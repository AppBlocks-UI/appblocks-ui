...
import { SearchButton } from '@root/components/docs-elements/algolia-search'
...
function OptionText({ searchWords, textToHighlight }: OptionTextProps) {
  ...
  const highlightedText = chunks.map((chunk: any, index: number) => {
    const { end, highlight, start } = chunk
    const text = textToHighlight.substr(start, end - start)
    if (highlight) {
      return (
        <Box key={`ht-${index}`} as='mark' bg='transparent' color='gray.l80'>
          {text}
        </Box>
      )
    } else {
          ...
          <Flex pos='relative' align='stretch'>
            <chakra.input
              ...
              sx={{
                ...
                outline: 0,
                bg: 'white',
                '.chakra-ui-dark &': { bg: 'gray.l10', color: 'gray.l80' },
              }}
